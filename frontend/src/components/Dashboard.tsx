...
export default function Dashboard() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ✅ Use the dedicated createBucket function (PUT request to /api/v1/bucket/{name})
      await createBucket(bucketName.trim());

      // The Rust server returns 200 OK for success or if it already exists.
  ...
  return (
    <main className="max-w-5xl mx-auto py-8 px-6 space-y-8">
      ...
      <section className="bg-white rounded-lg shadow w-full p-6">
        <h2 className="text-xl font-semibold mb-4 text-indigo-700">🪣 Create New Bucket</h2>
